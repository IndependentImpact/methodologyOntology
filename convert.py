...
def detect_rdf_format(input_path: str) -> Optional[str]:
...
def convert_rdf(input_path: str) -> None:
    # Load RDF input (TTL/OWL/JSON-LD supported)
    g = Graph()
    rdf_format = detect_rdf_format(input_path)
    g.parse(input_path, format=rdf_format)
    ...
    print(f"Saved RDF/XML to {owl_path}")
